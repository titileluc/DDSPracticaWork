...
tryCatch(
    expr = {
        ...
        features <- rbind(features, c("AttackCat", "chr", "symbolic"
        ...
        features <- rbind(features, c("IsAttack", "boolean", "symbolic"
                                      , "Boolean flag indicating wether an attack was observed." ))
        ...
        features <- rbind(features, c("IsDosAttack", "boolean", "symbolic"
                                      , "Boolean flag indicating wether a DOS attack was observed." ))
        ...
        features <- rbind(features, c("IsProbeAttack", "boolean", "symbolic"
                                      , "Boolean flag indicating wether a Probe attack was observed." ))
        ...
        features <- rbind(features, c("IsR2Lattack", "boolean", "symbolic"
                                      , "Boolean flag indicating wether a R2L attack was observed." ))
        ...
        features <- rbind(features, c("IsU2Rattack", "boolean", "symbolic"
                                      , "Boolean flag indicating wether a U2R attack was observed." ))
        ...
        features <- rbind(features, c("IsOtherAttack", "boolean", "symbolic"
                                      , "Boolean flag indicating wether an unclassified (other) attack was observed." ))
...
```{r raw_correlations, cache = TRUE}
# Select columns with non-zero standard deviation
...
corrs <- corrs[order(desc(corrs$coefficient), corrs$pvalue), 1:3]
#knitr::kable(corrs, row.names = FALSE)
```


#### Identification of the variables correlated with the <i><b>generic</b></i> 'IsAttack' Boolean flag indicating wether an attack was observed.

```{r IsAttack_correlations, include=FALSE}
knitr::kable(corrs[corrs$feature2 == 'IsAttack',],  row.names = FALSE)
```


#### Identification of the variables correlated with the generic 'IsDosAttack' Boolean flag indicating wether a DOS attack was observed.
...
```{r IsDosAttack_correlations, include=FALSE}
knitr::kable(corrs[corrs$feature2 == 'IsDosAttack',],  row.names = FALSE)
```


#### Identification of the variables correlated with the generic 'IsProbeAttack' Boolean flag indicating wether a Probe attack was observed.
...
```{r IsProbeAttack_correlations, include=FALSE}
knitr::kable(corrs[corrs$feature2 == 'IsProbeAttack',],  row.names = FALSE)
```


#### Identification of the variables correlated with the generic 'IsR2LAttack' Boolean flag indicating wether a Remote-to-Local attack was observed.
...
```{r IsR2LAttack_correlations, include=FALSE}
knitr::kable(corrs[corrs$feature2 == 'IsR2Lattack',],  row.names = FALSE)
```


#### Identification of the variables correlated with the generic 'IsU2RAttack' Boolean flag indicating wether a User-to-Root attack was observed.
...
```{r IsU2RAttack_correlations, include=FALSE}
knitr::kable(corrs[corrs$feature2 == 'IsU2Rattack',],  row.names = FALSE)
```


#### Identification of the variables correlated with the generic 'IsOtherAttack' Boolean flag indicating wether an Other type of attack was observed.
...
```{r IsOtherAttack_correlations, include=FALSE}
knitr::kable(corrs[corrs$feature2 == 'IsOtherAttack',],  row.names = FALSE)
```


#### Identification of the correlations between the other fields, the ones that are not response variables, in order to identify potential features to group into a single one. 

```{r other_fields_correlations, include=FALSE}
exclude_fields <- c('IsAttack', 'IsDosAttack', 'IsProbeAttack', 'IsR2Lattack', 'IsU2Rattack', 'IsOtherAttack')
knitr::kable(corrs[!corrs$feature2 %in% exclude_fields,],  row.names = FALSE)
```


## EXPLORATION OF THE TRAINING DATASET

With this section, the intention has been to try to automate "a first glimpse" of the data so that we could eventually reuse it.

In summary what it does is looping through all the dataset features in order for each to identify:
        ...
        if(minval == maxval){ 
            ...
            if (features[i, 3] == 'symbolic'){
                ...
                if( i < 42){
                    ...
                    cat(paste('\n   -> Chi-Square test between', current_feature, 'and the response variables.\n'
                        , '     To have evidence of association between the features,'
                        , 'we need the P-Values > 0 and <= 0.05\n'), fill = TRUE)
                ...
                if(stdev == 0){
                    ...
                    if( i < 42){
                        ...
                        cat(paste('\n   -> Correlation test between', current_feature
                            , 'and the numeric response variables.\n      To assume association between the features,'
                            , 'we need\n         - a Pearson coefficient >= 0.3 for a moderate correlation,'
                            , '\n         - >= 0.7 for a strong one'
                            , '\n         - and always with the P-Values > 0 and <= 0.05\n'), fill = TRUE)
